...
텔레그램/디스코드로 발송합니다. 웹에서는 소셜 로그인(카카오/네이버/구글/애플) 또는
이메일로 가입해 브리핑을 열람하고, 키워드를 구독·관리하며 텔레그램/디스코드 계정을
연결할 수 있습니다.
...
백엔드 (배치/봇)
  ...
  ├─ start.py      : 스케줄러 + 텔레그램 봇 + 디스코드 봇 통합 실행 (비정상 종료 시 재시작)
  ├─ scheduler.py  : 사용자별 발송 일정 스케줄러 (매분 체크, --once/--dry-run)
  └─ bots/         : 텔레그램 봇(/subscribe, /unsubscribe, /list, /brief, /start <code>)
                     + 디스코드 봇(/subscribe, /brief 등 슬래시 + ! 접두사 명령어)
```
...
- 토픽별 상이한 발송 일정 (현재는 사용자당 1개로 전체 토픽 공통)
- 사용자별 타임존 (KST 고정), Teams 연동
